...
no-key fallback、图表 provenance、canonical 自动推进和 central claim merge。旧的 `continuous` /
`acceptance` 只在入口作为兼容输入 alias，统一归一化为 `canonical`，不是两条产品路线。

## Canonical progression

orchestrator 是唯一用户入口。普通阶段 handoff、ready-unit 批次和跨阶段 cycle 在输入齐备时自动推进；
不因阶段边界额外要求人工验收。唯一的流程暂停标记是 `HUMAN_ACTION_REQUIRED`，只用于授权缺失、重大
歧义、需要科学判断或硬阻塞，并且必须写入可恢复的 `next_action`。自动推进永远不能越过来源、证据、
科学诚信或交付资格门。
...
2. 在用户明确的项目根目录检查 `workflow-state.md`、`review-intent.md` 和 `domain-profile.md`；可执行的最小文件编排器见 [orchestrator.py](orchestrator.py)。完成标准：三个资产全部存在且 frontmatter 可读，或已确认这是没有状态的全新项目。Grill 的完整输入还必须保留研究者语境、证据标准和边界场景；缺失时保持 `ACTIVE`，不得猜测或确认。
...
| Phase | 这一阶段必须解决的问题 |
| --- | --- |
| Grill | 研究问题、范围、价值假设、目标读者/期刊、研究者语境、证据标准、边界场景和排除项是否清楚？ |
...
- 需要用户配置 API、提供授权 PDF、解决受限来源、裁决重大歧义或作出科学判断时，输出唯一的流程停点 `HUMAN_ACTION_REQUIRED`，说明缺失能力、影响阶段、最小动作、降级路线和恢复方式。
- 普通阶段 handoff 和 ready-unit 批次由 canonical cycle 自动推进；不存在另一个 acceptance/continuous 产品模式。旧 mode 名称仅用于兼容输入并归一化为 `canonical`。
