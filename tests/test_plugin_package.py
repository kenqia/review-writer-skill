...
class ChemicalReviewPluginTests(unittest.TestCase):
    def test_manifest_and_explicit_skill_pack(self):
    ...
    def test_projection_is_synchronized_and_contains_only_public_files(self):
    ...
    def test_resolution_fails_closed_when_any_public_skill_is_missing(self):
        with tempfile.TemporaryDirectory() as temporary:
            plugin = Path(temporary)
            (plugin / ".codex-plugin").mkdir()
            (plugin / ".codex-plugin" / "plugin.json").write_text(json.dumps({"name": "chemical-review", "version": "0.2.0", "skills": "./skills/"}), encoding="utf-8")
            with self.assertRaisesRegex(ValueError, "HUMAN_ACTION_REQUIRED.*skill"):
