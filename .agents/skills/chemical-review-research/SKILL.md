...
原始 PDF 是来源权威，解析输出只可作为带 page/section locator 的阅读辅助。只有 Agent/研究者核对原始 PDF 后，才能在 `evidence-notes.md` 写入 `SOURCE_FACT [identity @ locator]`；自动解析生成的 `SOURCE_EXCERPT` 不能直接成为正文事实。Research 不写 draft.md。
