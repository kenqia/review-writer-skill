...
3. 可复用产物：当前 `review-brief.md`、`review-brief.proposed.md`、术语或 advisory 报告的路径；
4. 证据与推理边界：哪些是 `USER_ANSWER`、`DEFAULT`、`UNKNOWN` 或模型建议，哪些不能被下游当作来源事实；
5. 未解决问题与决定：研究者已接受、拒绝、暂缓的事项和冲突；
...
新草案写入 `review-brief.proposed.md`，确认前不得静默覆盖 `review-brief.md`（do not silently overwrite）。确认后更新 canonical brief，同时在 `intent-result.md` 指向旧快照和本轮决定。研究者的直接编辑、拒绝或暂缓建议都要保留；不要用模型重写覆盖它们。
