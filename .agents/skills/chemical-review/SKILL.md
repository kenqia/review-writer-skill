...
description: "Run a lightweight, iterative workflow for producing a critical narrative chemical literature review from a topic or research idea."
disable-model-invocation: true
argument-hint: "一个化学综述主题或研究想法"
---
...
1. 读取 [WORKFLOW.md](WORKFLOW.md)（阶段、状态、Research/Prototype/PRD 行为和确认语义）与 [ASSET-TEMPLATES.md](ASSET-TEMPLATES.md)（意图、领域、工作流和阶段资产的字段及人类可读结构）；当本次运行要启动、恢复或校验资产时才读取它们。完成标准：两份参考文件均已读完，且当前运行采用的阶段词和字段来自同一份模板。
...
5. 本次阶段工作完成后更新下一动作、未决风险和恢复提示；Research 维护证据资产，Prototype 维护小样本结果，PRD 维护可修订蓝图，Issues/Implement 维护独立 unit 资产并只通过 orchestrator 合并单一内容源，Review 反馈按最早失效阶段回退。完成标准：重新加载三个核心资产和已存在的阶段资产能重建本次运行的阶段、修订号、ready units 和人类动作要求。
...
Research、Prototype、PRD、Issues 和 Implement 的执行契约见 [WORKFLOW.md](WORKFLOW.md)；Review 与反馈闭环在后续 tickets 中实现。本入口只暴露一个统一路由、状态恢复、确认和中央合并边界。
