...
1. 读取 [WORKFLOW.md](WORKFLOW.md)（阶段、状态和确认语义）与 [ASSET-TEMPLATES.md](ASSET-TEMPLATES.md)（三个 Markdown 资产的字段和人类可读结构）；当本次运行要启动、恢复或校验资产时才读取它们。完成标准：两份参考文件均已读完，且当前运行采用的阶段词和字段来自同一份模板。
...
5. 本次阶段工作完成后更新下一动作、未决风险和恢复提示；Review 反馈按最早失效阶段回退。完成标准：重新加载三个资产能重建本次运行的阶段、意图修订号和人类动作要求。
