...
try:
    from .plugin_boundary import assert_exact_files, expected_release_files, relative_files
except ImportError:
    from plugin_boundary import assert_exact_files, expected_release_files, relative_files
...
def release_files(plugin: Path = PLUGIN):
    files = relative_files(plugin)
    assert_exact_files(files, expected_release_files(), label="release")
